...
<title>${user!=null && user.user_id!=''?'编辑':'添加'}注册用户信息</title>
...
<body>
<div class="pageTitle">
	&nbsp;&nbsp;<img src="images/right1.gif" align="middle" /> &nbsp;<span id="MainTitle" style="color:white">注册用户管理&gt;&gt;${user!=null && user.user_id!=''?'编辑':'添加'}注册用户</span>
</div>
     ...
     <td height="24">
       <Table border="0" cellspacing="0" cellpadding="0" align="center" width="100%"> 
            <TR>
              ...
              <TD class="edittitle">${user!=null && user.user_id!=''?'编辑':'添加'}注册用户</TD>
   ...
   <tr>
     ...
     <table width="100%" align="center" cellpadding="1" cellspacing="1" class="editbody">
       <tr>
          ...
          <td width="35%" >
          	<c:if test="${user!=null && user.user_id!=''}">${user.user_name}</c:if>
          	<c:if test="${user==null || user.user_id==''}"><input type="text" name="user_name" id="user_name" value="${user.user_name }"/> </c:if>
          </td>
          <td align="right" style="padding-right:5px"><font color="red">*</font> 密码：</td>
          <td>
            <c:if test="${user!=null && user.user_id!=''}">
        ...
        <tr>
          ...
          <td>
            <c:if test="${user!=null && user.user_id!=''}">${user.real_name}</c:if>
            <c:if test="${user==null || user.user_id==''}"><input type="text"  name="real_name" id="real_name" value="${user.real_name}"/></c:if>
          </td>
